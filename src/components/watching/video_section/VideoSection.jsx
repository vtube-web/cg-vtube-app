...
import {BiBell, BiDislike, BiLike, BiSolidDislike, BiSolidLike,} from "react-icons/bi";
...
function VideoSection({video}) {
    ...
    const handleSubscribeClick = (id) => {
        try {
            if (loggerUser) {
                if (isSubscribed) {
                    setSubcribeNumber(subcribeNumber - 1);
                    dispatch(removeSubscribed(id));
                } else {
                    setSubcribeNumber(subcribeNumber + 1);
                ...
                setReRender(!reRender);
                toast.success(isSubscribed ? "Subscription removed" : "Subscription added", {
    ...
    return (<>
            {video ? (<div className={`${style.watching__video}`}>
                <div className={style.video__container}>
                    {video.videoUrl && (<div className={style.custom__video__container}>
                        <ReactPlayer
                            url={video.videoUrl}
                ...
                <div className={`${style.video__info__function}`}>
                  ...
                  <span className={style.channel__name}>
                    {userDto.channelName ? userDto.channelName : userDto.userName}
                  </span>
                            </Link>
                            <span className={style.channel__subscribers}>
                  {formatNumberView(subcribeNumber)}
                                subscribers
                </span>
                        </div>
                        <ToastContainer/>
                        {!isVideoOwner(video.id) ? (<button
                            className={`${style.function__subscribe} ${isSubscribed ? style.unsubscribed : ""}`}
                            onClick={() => handleSubscribeClick(userDto.id)}
                        >
                            {isSubscribed ? (<span style={{display: "flex"}}>
                      <BiBell size={20}/> Subscribed
                    </span>) : (<span>Subscribe</span>)}
                        </button>) : (<button className={style.function__manage}>Manage video</button>)}
                    </div>
                    <div className={style.function__channel}>
                        ...
                        >
                            {isLiked ? (<BiSolidLike className={`${style.like__icon__liked} `}/>) : (
                                <BiLike className={style.like__icon}/>)}
                            <span>
                                    {isLiked ? formatNumberView(video.likes + 1) : formatNumberView(video.likes)}
                </span>
                        </button>
                        <button
